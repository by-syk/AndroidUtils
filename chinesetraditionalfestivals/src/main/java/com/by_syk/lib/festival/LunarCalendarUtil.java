...
class LunarCalendarUtil {
     ...
     * @return 返回农历日期对应的公历日期（精确到日），-1 为无效
     */
    public static long lunarToSolar(int year, int month, int monthDay, boolean isLeapMonth) {
        ...
        int dayOffset = (LUNAR_INFO[year - MIN_YEAR] & 0x001F) - 1;
        ...
        for (int i = 1; i <= 12; ++i) {
            ...
            if (year % 4 == 0 && i == 2 && iPos + 1 == dayOffset) {
                ...
                calendar.set(Calendar.DAY_OF_MONTH, dayOffset - 31);
                break;
            }
            ...
            if (iPos >= dayOffset) {
                ...
                if (dayOffset > iPos) {
                    calendar.set(Calendar.DAY_OF_MONTH, dayOffset - iPos);
                } else if (dayOffset == iPos) {
                    if (year % 4 == 0 && i == 2) {
                        calendar.set(Calendar.DAY_OF_MONTH, DAYS_BEFORE_MONTH[i] - DAYS_BEFORE_MONTH[i - 1] + 1);
                    } else {
                        calendar.set(Calendar.DAY_OF_MONTH, DAYS_BEFORE_MONTH[i] - DAYS_BEFORE_MONTH[i - 1]);
                    }
                } else {
